...
class UserDevice(object):
    ...
    def __init__(self, suuid: str = None, user_uuid: str = None, pin_code: int = None, device_token: str = None,
...
class UserDeviceStored(StoredObject, UserDevice):
    ...
    def __init__(self, storage_service: StorageService, suuid: str = None, user_uuid: str = None, pin_code: int = None,
        ...
        UserDevice.__init__(self, suuid=suuid, user_uuid=user_uuid, pin_code=pin_code, device_token=device_token,
...
class UserDeviceDB(UserDeviceStored):
    ...
    def update_by_pin_code(self):
            logging.info('Update UserDevice by pin_code')
            ...
            try:
                ...
                try:
                    ...
                    pass
                error_message = AuthError.USER_DEVICE_UPDATE_BYPINCODE_ERROR_DB.message
                developer_message = "%s. DatabaseError. Something wrong with database or SQL is broken. " \
                                    "Code: %s . %s" % (
                                        AuthError.USER_DEVICE_UPDATE_BYPINCODE_ERROR_DB.developer_message, e.pgcode, e.pgerror)
                error_code = AuthError.USER_DEVICE_UPDATE_BYPINCODE_ERROR_DB.code
                raise UserDeviceException(error=error_message, error_code=error_code,
    ...
    def __map_user_devicedb_to_user_device(self, user_device_db):
        return UserDevice(suuid=user_device_db[self._suuid_field], user_uuid=user_device_db[self._user_uuid_field],
                          pin_code=user_device_db[self._pin_code_field],
    ...
    def find_by_pincode(self) -> Optional[UserDevice]:
        logging.info('Find UserDevice by pin_code')
        select_sql = '''
                        SELECT 
                            uuid,
                            user_uuid,
        ...
        try:
            ...
            logging.error(e)
            error_message = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR_DB.message
            error_code = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR_DB.code
            developer_message = "%s. DatabaseError. Something wrong with database or SQL is broken. " \
                                "Code: %s . %s" % (
                                    AuthError.USER_DEVICE_FINDBYPINCODE_ERROR_DB.developer_message, e.pgcode,
                                    e.pgerror)
            raise UserDeviceException(error=error_message, error_code=error_code, developer_message=developer_message)
        ...
        if len(user_device_list_db) == 1:
            ...
            error_message = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR.message
            error_code = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR.code
            developer_message = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR.developer_message
            raise UserDeviceNotFoundException(error=error_message, error_code=error_code,
                                              developer_message=developer_message)
        else:
            error_message = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR.message
            developer_message = "%s. Find by specified email return more than 1 object. This is CAN NOT be! Something " \
                                "really bad with database." % AuthError.USER_DEVICE_FINDBYPINCODE_ERROR.developer_message
            error_code = AuthError.USER_DEVICE_FINDBYPINCODE_ERROR.code
            raise UserDeviceException(error=error_message, error_code=error_code, developer_message=developer_message)
